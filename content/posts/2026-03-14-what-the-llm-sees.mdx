...
summary: "LLMs have no memory. Every turn, your app reconstructs the entire conversation and sends it from scratch. Here's what that array looks like — and what happens when it gets too long."
---

The model has no memory. Between calls, it holds no state. When you send your second message, the API has no idea what you said in the first one — unless your app re-sends it.

What you think of as a "conversation" is your application serialising every prior exchange into a list and re-uploading the whole thing on every single request. The model sees a fresh document each time. It reconstructs the conversation from that list, generates the next token, and forgets everything.

This is the messages array. Understanding it is the foundation of building anything serious with LLMs.
...
The expensive step is `POST /messages (full array, every turn)`. Your app is the one responsible for maintaining, loading, and re-uploading the conversation history. The API is stateless by design — it just takes an array and returns a completion.

This is a feature. It means you have full control over what the model sees. You can redact messages, inject context mid-conversation, summarise old turns, or swap in retrieved documents. The model can't tell the difference between an "authentic" history and one your app constructed. That flexibility is what makes agents possible.
...
| Claude Sonnet 4.6 | 1M tokens | Balanced; surcharge above ~32k |
| Claude Opus 4.6 | 1M tokens | Most capable; surcharge above ~32k |
| GPT-5.4 | 128k tokens | Competitive |

1M tokens sounds generous until you see the surcharge. Most providers charge 2–4× more per token above the ~32k threshold. The extended window is there when you need it; it's not free. A typical English word is about 1.3 tokens, so 200k tokens is already a long novel. Budget pressure arrives long before the hard limit.
...
**Context compaction and stateful APIs** are the opt-out. Anthropic's API now supports automatic compaction: when the context nears the limit, the model summarises older turns server-side and continues the thread. OpenAI's Responses API is stateful by default — you send only the new message; the server holds the history. Both eliminate the reconstruct-and-re-upload loop described at the top of this post. The tradeoff: you no longer control what the model sees. You can't redact a turn, inject context mid-session, or inspect the compressed history. For most chatbots, that's fine. For agents it usually isn't.
...
The context window is the most concrete, measurable resource in an LLM application. Everything else — prompt quality, temperature, model choice — is softer. This one is arithmetic. Know what you're sending, track how it grows, and have a plan for when it doesn't fit anymore.
